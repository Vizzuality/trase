class MapController < ApplicationController
  ...
  def index
    ...
    dimensions = ContextLayer.select('context_layer.id as id, layer_attribute_id, COALESCE (inds.frontend_name, quants.frontend_name) as name, COALESCE (inds.tooltip_text, quants.tooltip_text) as description, lower(layer_attribute_type::text) as type, bucket_3, bucket_5, context_layer.context_layer_group_id as group_id, COALESCE (inds.unit, quants.unit) as unit, is_default')
