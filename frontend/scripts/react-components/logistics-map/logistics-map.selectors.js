...
export const getHeading = createSelector([getSelectedCommodity], commodity => {
  if (commodity === 'palmOil') return 'palm oil facilities';
  return commodity === 'soy' ? 'soy facilities' : 'slaughterhouses';
});

export const getBounds = createSelector([getSelectedCommodity], commodity => {
  const bounds = {
    brazil: {
...
const getSelectedTemplates = createSelector([getSelectedCommodity], commodity => {
...
export const getLogisticsMapYearsProps = createSelector([getActiveParams], activeParams => {
