...
export function* getUnitLayerData(params) {
  ...
  const url = `//p2cs-sei.carto.com/api/v2/sql?q=SELECT node_id, geo_id, attribute_id, json_object_agg(COALESCE(year, 0 ), total ORDER BY year) as years FROM (SELECT node_id, geo_id, attribute_id, year, SUM(value) AS total FROM map_attributes_values WHERE node_type_id = ${selectedGeoColumnId} and attribute_id IN (${selectedUnitIndicatorIds.join(',')}) and iso2 = '${iso2}' GROUP BY node_id, geo_id, attribute_id, year) s GROUP BY node_id, geo_id, attribute_id ORDER BY geo_id, node_id, attribute_id`;
  const { source, fetchPromise } = fetchWithCancel(url);
  try {
    const { data } =  yield call(fetchPromise);
