...
const Footer = () => (
  <div className="wrap">
    <div className="contain-logos-footer">
      {
        sections.map(({ title, logos, text }) => (
          <div key={title} className="contain-logos">
            <h4 className="title -mono-font">{title}</h4>
            {
              logos.map(({ list, listClassName }, listIndex) => (
                <ul key={title + listIndex} className={cx('logo-list', listClassName)}>
                  {
                    list.map(logo => (
    ...
    <div className="contain-link-list">
