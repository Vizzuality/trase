...
export const goToNodeProfilePage = node => dispatch => dispatch({
  type: `profile${capitalize(node.profileType)}`,
  payload: { query: { nodeId: node.id, year: DEFAULT_PROFILE_PAGE_YEAR } }
});

export const loadProfileSearchNodes = () => (dispatch) => {
  ...
  fetch(allNodesURL)
    ...
    .then((result) => {
      ...
      dispatch({
        type: SET_PROFILE_SEARCH_NODES, payload: { nodes: nodesArray }
      });
    })
    .catch(
      (reason) => {
