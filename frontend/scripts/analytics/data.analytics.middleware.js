...
import {
  GA_TRACK_DOWNLOAD_FILE_TYPE, GA_TRACK_DOWNLOAD_FILTERS, GA_TRACK_DOWNLOAD_FORM_LOADED,
...
export const GA_ACTION_WHITELIST = [{
  ...
  getPayload: (action, state) => {
    const context = state.data.contexts.find(elem => elem.id === parseInt(action.payload.context_id, 10));
    const payload = Object.assign({}, {
      country: context.countryName,
      commodity: context.commodityName
    }, action.payload);
    if (payload.countries_ids) {
      payload.consumptionCountries = payload.countries_ids
        .map(countryId => state.data.consumptionCountries.find(elem => elem.id === parseInt(countryId, 10)).name);
    }
    if (payload.exporters_ids) {
      payload.exporters = payload.exporters_ids
        .map(exportersId => state.data.exporters.find(elem => elem.id === parseInt(exportersId, 10)).name);
    }
    if (payload.indicators) {
      payload.indicators = payload.indicators
        .map(indicatorName => state.data.indicators.find(elem => elem.name === indicatorName).frontendName);
    }
