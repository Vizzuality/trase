...
export default class {
  ...
  _parseData() {
    this.data.forEach((d) => {
      ...
      d.value = _.isArray(d.values)
        ? formatValue(d.values[0] * 100, 'percentage')
        : formatValue(d.value * 100, 'percentage');
      d.link = this.targetLink && !d.is_domestic_consumption
        ? `/profile-${this.targetLink}?nodeId=${d.id}&year=${this.year}`
        : null;
    });
  }
