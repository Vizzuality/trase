...
The configuraton export dashboard is under [Database -> Configuration export](https://sandbox.trase.earth/content/admin/configuration_export).
...
```
curl -X POST -H "Content-Type: application/json" -d '{"email":"xxx@xxx.xxx","password":"xxxxxxxx"}' -i https://sandbox.trase.earth/api/private/auth/sign_in
```
...
```
curl -X POST -H "Content-Type: application/json" -d '' -H "access-token:ACCESS_TOKEN" -H "uid:UID" -H "token-type:Bearer" -H "client:CLIENT" https://sandbox.trase.earth/api/private/configuration_export_events
```
...
```
curl -H "Content-Type: application/json" -H "access-token:ACCESS_TOKEN" -H "uid:UID" -H "token-type:Bearer" -H "client:CLIENT" https://sandbox.trase.earth/api/private/configuration_export_events/5
```
