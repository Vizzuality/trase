...
Remember about a few IMPORTANT customisations:

- ensure the backend port is set to the application server port (8080)
- increase the default .between_bytes_timeout to 60s, to avoid "guru meditation"
- ensure /health requests always pass to the backend
  ````
  if (req.url ~ "^/health") {
    return (pass);
  }
  ````
- ensure BAN requests are handled
- downloads and webshot are also configured to pass to the backend
