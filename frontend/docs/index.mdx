...
UNIT_LAYERS_API_URL: URL of the vector tiles of the unit layers // Only sandbox and production are available
UNIT_LAYERS_DATA_ENV: Environment for the carto table that stores the unit layers data
...
USE_SERVICE_WORKER=Register service worker to cache the data on subsequent visits
DATA_DOWNLOAD_ENABLED=Enable data download in downloads page
ENABLE_INTERSECTION_OBSERVER=Enable showing content using intersection observer
ENABLE_VERSIONING=Enable versioning modal on tool
ENABLE_TOOL_PANEL=
ENABLE_LOGISTIC_LAYERS_TAB=Enable logistic layers tab on layer selection on tool
ENABLE_GOOGLE_TRANSLATE=Enable google translation
ENABLE_POPUP=Enable disclaimer pop-up on the whole app
```
...
- Context: Model based on the association of one country and one commodity. eg. Brazil-Soy
...
We are using transifex service imported with a script into the index.ejs template.
There have been some race conditions and bugs related to the loading of transifex: See transifex utils.
