SELECT * FROM (
  ...
  LEFT JOIN (
    SELECT quant_id, JSONB_OBJECT_AGG(context_id, tooltip_text) AS tooltip_text_by_context_id
    FROM quant_context_properties
    GROUP BY quant_id
  ) context_p ON quants.id = context_p.quant_id
  LEFT JOIN (
    SELECT quant_id, JSONB_OBJECT_AGG(country_id, tooltip_text) AS tooltip_text_by_country_id
    FROM quant_country_properties
    GROUP BY quant_id
  ) country_p ON quants.id = country_p.quant_id
  LEFT JOIN (
    SELECT quant_id, JSONB_OBJECT_AGG(commodity_id, tooltip_text) AS tooltip_text_by_commodity_id
  ...
  LEFT JOIN (
    SELECT ind_id, JSONB_OBJECT_AGG(context_id, tooltip_text) AS tooltip_text_by_context_id
    FROM ind_context_properties
    GROUP BY ind_id
  ) context_p ON inds.id = context_p.ind_id
  LEFT JOIN (
    SELECT ind_id, JSONB_OBJECT_AGG(country_id, tooltip_text) AS tooltip_text_by_country_id
    FROM ind_country_properties
    GROUP BY ind_id
  ) country_p ON inds.id = country_p.ind_id
  LEFT JOIN (
    SELECT ind_id, JSONB_OBJECT_AGG(commodity_id, tooltip_text) AS tooltip_text_by_commodity_id
  ...
  LEFT JOIN (
    SELECT qual_id, JSONB_OBJECT_AGG(context_id, tooltip_text) AS tooltip_text_by_context_id
    FROM qual_context_properties
    GROUP BY qual_id
  ) context_p ON quals.id = context_p.qual_id
  LEFT JOIN (
    SELECT qual_id, JSONB_OBJECT_AGG(country_id, tooltip_text) AS tooltip_text_by_country_id
    FROM qual_country_properties
    GROUP BY qual_id
  ) country_p ON quals.id = country_p.qual_id
  LEFT JOIN (
    SELECT qual_id, JSONB_OBJECT_AGG(commodity_id, tooltip_text) AS tooltip_text_by_commodity_id
