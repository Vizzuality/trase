shared_context 'quants' do
  let(:area) do
    Quant.find_by_name('AREA_KM2') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: true, place_factsheet_temporal: false, name: 'AREA_KM2', unit: 'km2', unit_type: 'area', tooltip_text: 'Municipal area according in Km2', frontend_name: 'Area')
  end
  let(:land_conflicts) do
    Quant.find_by_name('LAND_CONFL') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: true, place_factsheet_temporal: false, name: 'LAND_CONFL', unit: 'Number', unit_type: 'count', tooltip_text: 'Number of land conflicts per municipality', frontend_name: 'Land conflicts (2014)')
  end
  let(:force_labour) do
  ...
  let(:fob) do
    Quant.find_by_name('FOB') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: true, place_factsheet_temporal: false, name: 'FOB', unit: 'USD', tooltip_text: 'Value of commodity shipments', frontend_name: 'Financial flow')
  end
  ...
  let(:population) do
    Quant.find_by_name('POPULATION') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: true, place_factsheet_temporal: false, name: 'POPULATION', unit: 'Number', tooltip_text: 'Population per municipality', frontend_name: 'Population')
  end
  let(:soy_tn) do
    Quant.find_by_name('SOY_TN') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: true, place_factsheet_temporal: true, name: 'SOY_TN', unit: 'Tn', tooltip_text: 'Production of soy in Tn.', frontend_name: 'Production of soy')
  end
  ...
  let(:soy_) do
    Quant.find_by_name('SOY_') ||
      FactoryBot.create(:quant, actor_factsheet: true, place_factsheet: false, place_factsheet_temporal: false, name: 'SOY_', unit: 'Tn', tooltip_text: 'Total exports in Tn of soy equivalents', frontend_name: 'Soy exports for trader')
  end
  let(:agrosatelite_soy_defor_) do
    Quant.find_by_name('AGROSATELITE_SOY_DEFOR_') ||
      FactoryBot.create(:quant, actor_factsheet: true, place_factsheet: true, place_factsheet_temporal: true, name: 'AGROSATELITE_SOY_DEFOR_', unit: 'Tn', tooltip_text: 'Annual deforestation due to direct conversion for soy (ha) (currently only in Cerrado biome, 2010-2013). Calculated by crossing per-pixel annual deforestation alerts and soy crop maps.', frontend_name: 'Soy deforestation (Cerrado only)')
  end
  let(:volume) do
    Quant.find_by_name('Volume') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: false, place_factsheet_temporal: false, name: 'Volume', unit: 'Tn', frontend_name: 'Trade volume')
  end
  let(:land_use) do
    Quant.find_by_name('LAND_USE') ||
      FactoryBot.create(:quant, actor_factsheet: false, place_factsheet: false, place_factsheet_temporal: false, name: 'LAND_USE', unit: 'Ha', frontend_name: 'Land use')
  end
  let(:biodiversity) do
    Quant.find_by_name('BIODIVERSITY') ||
      FactoryBot.create(:quant, actor_factsheet: true, place_factsheet: true, place_factsheet_temporal: false, name: 'BIODIVERSITY', unit: 'Unitless', frontend_name: 'Loss of biodiversity habitat')
  end
  let(:ghg_) do
