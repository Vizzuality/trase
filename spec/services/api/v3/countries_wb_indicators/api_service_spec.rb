...
RSpec.describe Api::V3::CountriesWbIndicators::ApiService do
  ...
  context '#population_indicators' do
    ...
    it 'return world bank population indicators' do
      ...
      expect(indicator[:name]).to eql 'population'
  ...
  context '#gdp_indicators' do
    before do
      ...
      response = File.read(worldbank_request_path(:gdp))
      stub_request(:get, uri).to_return(body: response)
    end
    ...
    it 'return world bank gdp indicators' do
      ...
      indicator = indicators[:indicators].first
      expect(indicator[:iso_code]).to eql 'ARG'
      expect(indicator[:name]).to eql 'gdp'
      expect(indicator[:year]).to eql 2013
      expect(indicator[:value]).to eql 2_867_811_095_547.36
    end
  end
  ...
  context '#land_area_indicators' do
    before do
      ...
      response = File.read(worldbank_request_path(:land_area))
      stub_request(:get, uri).to_return(body: response)
    end
    ...
    it 'return world bank land_area indicators' do
      ...
      indicator = indicators[:indicators].first
      expect(indicator[:iso_code]).to eql 'ARG'
      expect(indicator[:name]).to eql 'land_area'
      expect(indicator[:year]).to eql 2013
      expect(indicator[:value]).to eql 11_232_639.0078735
    end
  end
  ...
  context '#agricultural_land_area_indicators' do
    before do
      ...
      response = File.read(worldbank_request_path(:agricultural_land_area))
      stub_request(:get, uri).to_return(body: response)
    end
    ...
    it 'return world bank agricultural_land_area indicators' do
      ...
      indicator = indicators[:indicators].first
      expect(indicator[:iso_code]).to eql 'ARG'
      expect(indicator[:name]).to eql 'agricultural_land_area'
      expect(indicator[:year]).to eql 2013
      expect(indicator[:value]).to eql 4_785_734.43078995
    end
  end
  ...
  context '#forested_land_area_indicators' do
    before do
      ...
      response = File.read(worldbank_request_path(:forested_land_area))
      stub_request(:get, uri).to_return(body: response)
    end
    ...
    it 'return world bank forested_land_area indicators' do
      ...
      indicator = indicators[:indicators].first
      expect(indicator[:iso_code]).to eql 'ARG'
      expect(indicator[:name]).to eql 'forested_land_area'
      expect(indicator[:year]).to eql 2013
      expect(indicator[:value]).to eql 386_635.796659589
    end
  end
end
