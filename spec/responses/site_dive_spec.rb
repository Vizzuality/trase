...
RSpec.describe 'Get a site dive', type: :request do
  ...
  describe 'GET /site dive' do
    it 'requires a site dive id' do
      expect { get '/api/v2/site_dive' }.to raise_error(ActionController::RoutingError)
    end

    it 'returns 404 on non-existent context_id' do
      expect { get "/api/v2/site_dive/#{site_dive_1.id + 1}" }.to raise_error(ActiveRecord::RecordNotFound)
    end

    it 'has the correct response structure' do
      get "/api/v2/site_dive/#{site_dive_1.id}"
